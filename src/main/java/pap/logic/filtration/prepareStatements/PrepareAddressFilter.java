...
public class PrepareAddressFilter {
    ...
    public String prepareQuery() {
        ...
        if (addressInfo.getCity() != null && !addressInfo.getCity().isEmpty()) {
            baseQuery.append(" AND LOWER(addresses.city) = LOWER('").append(addressInfo.getCity()).append("')");
        }

        if (addressInfo.getCountry() != null && !addressInfo.getCountry().isEmpty()) {
            baseQuery.append(" AND LOWER(addresses.country) = LOWER('").append(addressInfo.getCountry()).append("')");
        }

        if (addressInfo.getStreet() != null && !addressInfo.getStreet().isEmpty()) {
            baseQuery.append(" AND LOWER(addresses.street) = LOWER('").append(addressInfo.getStreet()).append("')");
        }

        if (addressInfo.getPostalCode() != null && !addressInfo.getPostalCode().isEmpty()) {
            baseQuery.append(" AND LOWER(addresses.postal_code) = LOWER('").append(addressInfo.getPostalCode()).append("')");
        }

        if (addressInfo.getStreetNo() != null && !addressInfo.getStreetNo().isEmpty()) {
            baseQuery.append(" AND LOWER(addresses.street_no) = LOWER('").append(addressInfo.getStreetNo()).append("')");
        }
